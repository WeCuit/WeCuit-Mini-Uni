...
const XMLParser = require("../xmldom/dom-parser.js");

const REQUEST = require("../request.js");

const xmlParser = new XMLParser.DOMParser();
var loginFailed = 0; // RSA加密
...
class LOGIN {
  ...
  webVpnLogin(auth) {
    return new Promise((resolve, reject) => {
      uni.request({
        ...
        header: {
          "content-type": "application/x-www-form-urlencoded",
          cookie: "language=zh_CN; privacy=1; ENABLE_RANDCODE=" + this.WEBVPN.isNeedCaptcha + "; TWFID=" + auth.TwfID
        },
  ...
  webVpnCheckLogin() {
    return new Promise((resolve, reject) => {
      uni.request({
        ...
        header: {
          cookie: "language=zh_CN; privacy=1; ENABLE_RANDCODE=" + this.WEBVPN.isNeedCaptcha + "; TWFID=" + this.session.TWFID
        },
  ...
  webVpnAutoLogin(r) {
    ...
    temp.then(() => {
      ...
      res["encrypted_pwd"] = encrypt.encrypt(this.account.vpnPass ? this.account.vpnPass : this.account.userPass + "_" + res.rc);
        ...
        success: res => {
          ...
          if (res.data.indexOf("已经成功登统一认证中心") != -1) {
            ...
            if (res.data.indexOf("execution") != -1) {
              this.SSO.execution = ret["execution"] = res.data.match(/" name="execution" value="(.*?)" \/>/)[1];
            }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        uni.request({
          ...
          data: pic,
          success: res => {
            if (2000 == res.data.status) resolve(res.data.result);else if (res.data.errMsg) {
              uni.showToast({
                icon: "none",
