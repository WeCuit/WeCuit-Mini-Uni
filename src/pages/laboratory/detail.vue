...
<view class="search">
    ...
    <form id="card-body-0" :style="'--card-height:' + (heightData['card-body-0']?heightData['card-body-0']:'auto')" :class="'card-body ' + (hideData['ele_0']?'hide':'display')" @submit="doSubmit">
        <view class="input-form">
            <view class="item">
                <view>学期：</view>
                <input type="text" name="Kkxq" data-name="Kkxq" @input="bindinput" v-model="Kkxq" maxlength="5" @focus="showNotice" @blur="hideNotice" data-title="可输入名称的一部分" confirm-type="search" @confirm="doSubmit2">
            </view>
            <view class="item">
                <view>院系：</view>
                <picker mode="selector" name="Yx" data-name="Yx" :range="Yx" range-key="text" v-model="Yx_index">
                    {{Yx[Yx_index].text}}
                </picker>
            </view>
            <view class="item">
                <view>任务：</view>
                <picker mode="selector" name="Rw" data-name="Rw" :range="Rw" range-key="text" v-model="Rw_index">
                    {{Rw[Rw_index].text}}
                </picker>
            </view>
            <view class="item">
                <view>实验室：</view>
                <input type="text" name="Sys" data-name="Sys" @input="bindinput" v-model="Sys" maxlength="16" confirm-type="search" @focus="showNotice" @blur="hideNotice" data-title="可输入名称的一部分" @confirm="doSubmit2">
            </view>
            <view class="item">
                <view>房间：</view>
                <input type="text" name="Fj" data-name="Fj" @input="bindinput" v-model="Fj" maxlength="8" confirm-type="search" @focus="showNotice" @blur="hideNotice" data-title="可只输入房间编号的左边部分" @confirm="doSubmit2">
            </view>
            <view class="item">
                <view>教学班：</view>
                <input type="text" name="Jxb" data-name="Jxb" @input="bindinput" v-model="Jxb" maxlength="10" confirm-type="search" @focus="showNotice" @blur="hideNotice" data-title="可只输入班简名的左边部分" @confirm="doSubmit2">
            </view>
            <view class="item">
                <view>教师/助教：</view>
                <input type="text" name="Zjjs" data-name="Zjjs" @input="bindinput" v-model="Zjjs" maxlength="10" confirm-type="search" @focus="showNotice" @blur="hideNotice" data-title="可只输入姓名的左边部分" @confirm="doSubmit2">
            </view>
            <view class="item">
                <view>课程：</view>
                <input type="text" name="Jxkc" data-name="Jxkc" @input="bindinput" v-model="Jxkc" maxlength="20" confirm-type="search" @focus="showNotice" @blur="hideNotice" data-title="可只输入课程名称的左边部分" @confirm="doSubmit2">
            </view>
            <view class="item">
                <view>时间范围：</view>
            ...
            <view class="item">
                <view>第</view>
                <input type="number" name="cxZc" data-name="cxZc" @input="bindinput" v-model="cxZc" maxlength="2" @confirm="doSubmit2">
                <view>周</view>
            </view>
            <view class="item">
                <view>类别：</view>
                <picker mode="selector" name="Lb" data-name="Lb" :range="Lb" range-key="text" v-model="Lb_index">
                    {{Lb[Lb_index].text}}
                </picker>
            </view>
        </view>
    ...
    <block v-for="(item, index) in retList" :key="index">
        <view class="search">
            <view class="title">
                ...
                <view @tap="hideAni" :data-ele="(1+index)" :class="'iconfont icon-' + (!hideData['ele_' + (1+index)]?'shouqi':'zhankai')"></view>
            </view>
                    ...
                    <block v-for="(lesson_item, lesson_index) in item" :key="lesson_index">
                        ...
                        <view class="lesson__body">
                            <block v-for="(plan_item, plan_index) in lesson_item" :key="plan_index">
								<!-- TODO: -->
                                <view class="plan__name" :style="{color:labDetail.getStyle(plan_item)}">{{plan_item}}</view>
                            </block>
                        </view>
                    </block>
                </view>
