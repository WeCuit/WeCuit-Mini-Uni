<template>
  <view>
    ...
    <view class="search">
      ...
      <form
        id="card-body-0"
        :style="'--card-height:' + (heightData[0]?heightData[0]:'auto')"
        ...
        <view class="input-form">
          ...
          <view class="item">
            ...
            <picker
              ...
              :range="cxXsms"
              range-key="text"
              v-model="cxXsms_index"
            >{{cxXsms[cxXsms_index].text}}</picker>
          </view>
        </view>
    ...
    <view class="result">
      <block v-if="retList.length>0">
        <view v-for="(item, index) in retList" :key="index" class="search">
          <view class="title">
            <navigator :url="'detail?' + item.link">{{item.name}}</navigator>
            <view
              @tap="hideAni"
              :data-ele="index + 1"
              :class="'iconfont icon-' + (!hideData['ele_' + (index+1)]?'shouqi':'zhankai')"
            ></view>
          </view>
          <view
            :id="'card-body-' + (index+1)"
            :style="'--card-height:' + (heightData[index+1]?heightData[index+1]:'auto')"
            :class="'card-body ' + (hideData['ele_' + (index+1)]?'hide':'display')"
          >
            <view class="place">
              <block v-for="(place_item, place_index) in item.place" :key="place_index">
                ...
                <navigator
                  class="place__title"
                  :url="'detail?' + place_item.link"
                >{{place_item.name}}</navigator>
                <view class="place__body">
                  <block v-for="(lab_item, lab_index) in place_item.lab" :key="lab_index">
                    ...
                    <navigator
                      class="lab__name"
                      :url="'detail?' + lab_item.link"
                      :style="lab_item.style"
                    >{{lab_item.name}}</navigator>
                  </block>
                </view>
              </block>
            </view>
