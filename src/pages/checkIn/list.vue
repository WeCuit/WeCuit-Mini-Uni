...
<mp-cells ext-class="my-cells" title="最新的打卡任务">
    <mp-cell v-for="(item, index) in list.today" :key="index" ext-class="my-cell" :data-link="item.link" @tap="openCheckIn">
        <view>{{item.status}} {{item.title}}</view>
        <!-- <view slot="footer">说明文字</view> -->
    </mp-cell>
</mp-cells>
<mp-cells ext-class="my-cells" title="已结束的打卡任务">
    <mp-cell v-for="(item, index) in list.outDate" :key="index" ext-class="my-cell" :data-link="item.link" @tap="openCheckIn">
        <view>{{item.status}} {{item.title}}</view>
        <!-- <view slot="footer">说明文字</view> -->
    </mp-cell>
</mp-cells>
    ...
    app.globalData.autoLoginProcess.then(() => {
      ...
      if (20401 == data.code) {
        ...
        if (app.globalData.isUser && this.isFirstTry) {
          ...
          app.globalData.loginClass.ccDoLogin().then(this.onPullDownRefresh);
        } else {
          uni.navigateTo({
            url: "../my/sso/sso"
          });
