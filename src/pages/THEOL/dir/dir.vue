...
<mp-cells title="目录" v-if="dir.folder">
    <mp-cell v-for="(item, index) in dir.folder" :key="index" ext-class="my-cell" :id="item.id" :data-courseId="item.courseId" @tap="bindViewDir">
        <view>{{item.text}}</view>
...
<view v-for="(item, index) in dir.file" :key="index" class="exam-card" :data-suffix="item.suffix" :data-fileid="item.id" :data-resid="item.resId" @tap="downloadFile">
...
export default {
  ...
  methods: {
    ...
    downloadFile: function (e) {
      ...
      var query = 'fileId=' + data.fileid + '&resId=' + data.resid + '&lid=' + this.lid + '&cookie=' + app.globalData.sessionInfo.theolCookie;
      uni.showLoading({
        title: '正在尝试下载'
      });
      uni.downloadFile({
