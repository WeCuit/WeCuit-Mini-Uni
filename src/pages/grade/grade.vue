...
<view v-for="(item, index) in grade" :key="index" style="margin-bottom: 2rem;">
  ...
  <view class="grade_table">
    <view v-for="(d, n) in item.data" :key="n" class="grade_table_item" :style="!check.isPass(d.learnGrade)?'background-color:red':''">
      <!-- 科目名称 -->
      <view>课程：{{n}}</view>
      <!-- 成绩详情 -->
      <view class="detail">
...
export default {
  ...
  methods: {
    gradeQueryV2: function () {
      getGradeTable(`${this.sessionInfo.JWGL_cookie}; TWFID=${this.sessionInfo.TWFID}`).then(res => {
        const resp = res.data;
    ...
    changeSubStatus: function (e) {
      app.globalData.httpPost({
        ...
        data: {
          ...
          sign: genQuerySign("/Task/gradeInfoV2/", app.globalData.openid, e.detail.value ? '1' : '0')
        }
      }).then(data => {
        uni.showToast({
          title: "修改成功"
