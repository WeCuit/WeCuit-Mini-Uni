...
<view v-if="isNeedLogin">
  ...
  <form catchsubmit="formSubmit" catchreset="formReset">
    <mp-cells ext-class="my-cells" title="请输入登录信息">
			<mp-cell title="学号：">
				<view><input name="userId" class="login weui-input" type="number" placeholder="请输入学号" placeholder-class="placeholder" :value="userId"></view>
			</mp-cell>
			<mp-cell title="密码：">
				<input name="userPass" class="login password" password="true" placeholder="请输入密码" placeholder-class="placeholder" :value="userPass">
			</mp-cell>
			<mp-cell title="WebVpn密码：">
				<input name="vpnPass" class="login password" password="true" placeholder="同上请放空" placeholder-class="placeholder" :value="vpnPass">
			</mp-cell>
...
<view v-else style="text-align:center;color:#a5ff2da6">
	...
	<view>WEBVPN<text v-if="isWebVpnLogin">已登录</text><text v-else data-type="WEBVPNV2" @tap="loginAction">未登录</text></view>
	<view>教务处<text v-if="isJwglLogin">已登录</text><text v-else data-type="JWGL" @tap="loginAction">未登录</text></view>
...
import { captchaOCR, getSSOCaptcha, checkSSOLogin, jwglLogin, ssoLogout, WV_login, WV_loginAuth, jwglLoginCheck, WV_loginCheck } from '../api'; // var L = new login.DoLogin();
// rsa 加密
  ...
  data() {
    return {
      ...
      loginFunc: {
        ...
        WEBVPN_isAdmin: function () {
          return new Promise((resolve, reject) => {
            // 普通用户
            if (true !== app.globalData.accountInfo.isAdmin) reject();else // 管理员
                ...
                uni.request({
                  ...
                  success: res => {
                    if (2000 === res.data.status) {
                      app.globalData.sessionInfo.TWFID = res.data.twfid;
                      uni.setStorage({
                        data: res.data.twfid,
                        key: "TWFID"
                      });
        ...
        WEBVPN: r => {
          new Promise((resolve, reject) => {
            _this.loginFunc.loginAuth(auth => {
              WV_login("language=zh_CN; privacy=1; ENABLE_RANDCODE=" + _this.isNeedCaptcha + "; TWFID=" + auth.TwfID, {
                mitm_result: "",
                ...
                var cookie = "";
                if ("undefined" != typeof res.header["set-cookie"]) cookie = res.header["set-cookie"];else if ("undefined" != typeof res.header["Set-Cookie"]) cookie = res.header["Set-Cookie"];
        ...
        loginAuth: function (r) {
          WV_loginAuth("language=zh_CN; privacy=1; ENABLE_RANDCODE=" + _this.isNeedCaptcha + ";TWFID=" + app.globalData.sessionInfo.TWFID) // new login.DoLogin().webVpnAuth(_this.data.isNeedCaptcha, app.sessionInfo.TWFID)
          .then(res => {
            var doc = xmlParser.parseFromString(res.data);
            var randCode = doc.getElementsByTagName("CSRF_RAND_CODE")[0].firstChild.data;
            var msg = doc.getElementsByTagName("Message")[0].firstChild.data;
            var TwfID = doc.getElementsByTagName("TwfID")[0].firstChild.data;
            var RSA_ENCRYPT_KEY = doc.getElementsByTagName("RSA_ENCRYPT_KEY")[0].firstChild.data;
            var RSA_ENCRYPT_EXP = doc.getElementsByTagName("RSA_ENCRYPT_EXP")[0].firstChild.data;
            let auth;
            ...
            ret["encrypted_pwd"] = encrypt.encrypt(_this.vpnPass ? _this.vpnPass : _this.userPass + "_" + ret.rc);
            r(ret);
          }).catch(err => {
            console.log(err);
      ...
      loginCheckFunc: {
        // WEBVPN检查登录状态
        WEBVPN: function () {
          const cookie = `language=zh_CN; privacy=1; ENABLE_RANDCODE=${_this.isNeedCaptcha}; TWFID=${app.globalData.sessionInfo.TWFID}`;
         ...
         */
        JWGL: function (r) {
          jwglLoginCheck(app.globalData.sessionInfo.JWGL_cookie + "; TWFID=" + app.globalData.sessionInfo.TWFID).then(res => {
  ...
  methods: {
    ...
    captchaDecode: function (pic, r) {
      try {
        ...
        captchaOCR(app.globalData.RSAEncrypt(verify), pic).then(res => {
          console.log(res);
          if (200 == res.data.code) r(res.data.data.result);else {
            uni.showToast({
              icon: "none",
              title: res.data.msg
            });
