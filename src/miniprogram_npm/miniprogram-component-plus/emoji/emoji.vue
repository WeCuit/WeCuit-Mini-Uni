  ...
  <view class="emotion_list" :style="'padding: 0 ' + extraPadding + 'px'">
    <view :hidden="!showHistory || history.length === 0">
      ...
      <block v-for="(item, index) in history" :key="index">
        <view class="emotion_item" :data-idx="item" @tap="insertEmoji">
    ...
    <view style="padding-left: 5px">所有表情</view>
      ...
      <block v-for="(item, index) in emotions" :key="index">
        <view class="emotion_item" :data-idx="index" @tap="insertEmoji">
          <view :class="'icon_emotion ' + item.style" :style="'background-image: url(' + emojiPicture + ')'"></view>
        </view>
      </block>
      <view v-if="showDel" class="emotion_item" @tap="deleteEmoji">
  ...
  Component({
    ...
    lifetimes: {
      attached: function attached() {
        var data = this.data;
        var areaWidth = uni.getSystemInfoSync().windowWidth;
