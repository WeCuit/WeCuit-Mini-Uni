<template>
<view>
<slot v-if="!nodes.length"></slot><trees class="top" :style="(selectable?'user-select:text;-webkit-user-select:text;':'') + '' + showAm" :lazy-load="lazyLoad" :loading="loadingImg" :nodes="nodes"></trees>
</view>
</template>
...
var e = {},
    i = require("./libs/MpHtmlParser.js"),
    n = uni.getFileSystemManager && uni.getFileSystemManager();
...
export default {
  ...
  created: function () {
    ...
    this.imgList = [], this.imgList.setItem = function (t, e) {
      ...
      if (t && e) {
        if (0 == e.indexOf("http") && this.includes(e)) {
          for (var s, a = "", o = 0; (s = e[o]) && ("/" != s || "/" == e[o - 1] || "/" == e[o + 1]); o++) a += Math.random() > .5 ? s.toUpperCase() : s;

          return a += e.substr(o), this[t] = a;
        }

        if (this[t] = e, e.includes("data:image")) {
          var r = e.match(/data:image\/(\S+?);(\S+?),(.+)/);
          if (!r) return;
          var l = uni.env.USER_DATA_PATH + "/" + Date.now() + "." + r[1];
          n && n.writeFile({
            filePath: l,
            data: r[3],
            encoding: r[2],
    ...
    }, dom && (this.document = new dom(this)), search && (this.search = function (e) {
      return search(t, e);
    });
  },
  destroyed: function () {
    this.imgList.each(function (t) {
      t && t.includes(uni.env.USER_DATA_PATH) && n && n.unlink({
        filePath: t
      });
  ...
  methods: {
    ...
    navigateTo: function (t) {
      ...
      this._in ? i.select(this._in.selector).fields({
        ...
        if (e._in) {
          var s = {};
          s[e._in.scrollTop] = n, e._in.page.setData(s);
    ...
    setContent: function (n, s) {
      ...
      var h = {};
      if (s) for (var c = this.nodes.length, d = a.length; d--;) h["nodes[" + (c + d) + "]"] = a[d];else h.nodes = a;
      this.showWithAnimation && (h.showAm = "animation: show .5s"), this.setData(h, function () {
        o.$emit("load");
      }), a.title && this.autosetTitle && uni.setNavigationBarTitle({
        title: a.title
      }), this.imgList.length = 0, this.videoContexts = [];

      for (var u, g = this.selectAllComponents(".top,.top>>>._node"), f = 0; u = g[f++];) {
        u.top = this;

        for (var m, p = 0; m = u.data.nodes[p++];) if (!m.c) if ("img" == m.name) this.imgList.setItem(m.attrs.i, m.attrs["original-src"] || m.attrs.src);else if ("video" == m.name || "audio" == m.name) {
          var v;
          v = "video" == m.name ? uni.createVideoContext(m.attrs.id, u) : u.selectComponent("#" + m.attrs.id), v && (v.id = m.attrs.id, this.videoContexts.push(v));
        }
      }

      var x;
      clearInterval(this._timer), this._timer = setInterval(function () {
        o.createSelectorQuery().select(".top").boundingClientRect(function (t) {
          t && (o.rect = t, t.height == x && (o.$emit("ready", {
            detail: t
          }), clearInterval(o._timer)), x = t.height);
        }).exec();
      }, 350);
    }
  }
...
<style>
:host{display:block;overflow:auto;webkit-overflow-scrolling:touch}.top{display:inherit}@keyframes show{0%{opacity:0}100%{opacity:1}}
</style>